...
import { crmListSchema, demandSchema } from "@/repositories/crm/crm-repository";
...
export async function GET(request: Request) {
  ...
  const demands = await getPrisma().politicalDemand.findMany({
...
export async function POST(request: Request) {
  ...
  const parsed = demandSchema.safeParse(await request.json());
  if (!parsed.success) return NextResponse.json({ error: "Parametros invalidos.", details: parsed.error.flatten() }, { status: 400 });

  await assertCampaignAccess(parsed.data.campaignId, guard.workspace.organization.id);
  const demand = await getPrisma().politicalDemand.create({
    data: {
      ...parsed.data,
