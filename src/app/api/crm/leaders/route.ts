...
import { crmListSchema, leaderSchema } from "@/repositories/crm/crm-repository";
...
export async function GET(request: Request) {
  ...
  const leaders = await getPrisma().politicalLeader.findMany({
...
export async function POST(request: Request) {
  ...
  const parsed = leaderSchema.safeParse(await request.json());
  if (!parsed.success) return NextResponse.json({ error: "Parametros invalidos.", details: parsed.error.flatten() }, { status: 400 });

  await assertCampaignAccess(parsed.data.campaignId, guard.workspace.organization.id);
  const leader = await getPrisma().politicalLeader.create({
    data: {
      ...parsed.data,
